...
def add():    
    ...
    with open('homeren.csv',mode='w',newline='') as file:
        writer= csv.writer(file)
        for i in range(len(projectid)):
            writer.writerow(("\t",projectid[i],"\t",name[i],"\t",des[i],"\t",start[i],"\t",end[i],"\t",duration[i],"\t",budget[i],"\t",status[i],"\t"))
def view():
    demo=0
...
while(count!=0):
    ...
    if(uname=="admin" and upass=="1234"):
        ...
        while(cnt!=0):
            ...
            print("5.Generate report by staus")
            print("exit")
...
try:
     with open('homeren.csv',mode='w',newline='') as file:
        ...
        for row in reader:
            print("\t projectid \t name \t des \t start \t end \t duration \t budget \t status \t")
            projectid.append(row[0])
            name.append(row[1])
            des.append(row[2])
            start.append(row[3])
            end.append(row[4])
